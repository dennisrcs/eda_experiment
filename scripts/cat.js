...
if (congruent) {
	...
	relaxedOrStressed = "relaxed"
}
else {
	checkBox1.checked = false;
	checkBox2.checked = false; // adaptive
	relaxedOrStressed = "stressed"
}
...
function startGame() {
	...
	localStorage.setItem("cwt_" + task_number + "_start_time", timestamp);
...
function durationUpdate() {
    var perc = 100 - Math.round((currTime/(duration*1000))*100);
      if (perc >= 0) {
		  ...
		  var timestamp = getFormattedDate();
		  localStorage.setItem("cwt_" + task_number + "_end_time", timestamp);
		  ...
		  window.location = "paced_breathing.html";
	  }
}
function failed() {
		currResponse = 0;
